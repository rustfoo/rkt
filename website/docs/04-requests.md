...
route. rkt ensures that this is the case before `handler` is called. Of
course, you can do much more than specify the method and path of a request.
Among other things, you can ask rkt to automatically validate:
...
validations. rkt's code generation takes care of actually validating the
properties. This section describes how to ask rkt to validate against all of
these properties and more.

[`route`]: https://docs.rs/rkt/latest/rkt/attr.route.html

## Methods

A rkt route attribute can either be method-specific, any one of `get`, `put`,
...
[`Method`]: https://docs.rs/rkt/latest/rkt/http/enum.Method.html

### HEAD Requests

rkt handles `HEAD` requests automatically when there exists a `GET` route
...
request by declaring a route for it; rkt won't interfere with `HEAD` requests
your application explicitly handles.

### Reinterpreting

Because web browsers only support submitting HTML forms as `GET` or `POST` requests,
rkt _reinterprets_ request methods under certain conditions. If a `POST`
...
trait. We call these types _parameter guards_. rkt implements `FromParam` for
many of the standard library types, as well as a few special rkt types. For
...
[`FromParam` API docs]: https://docs.rs/rkt/latest/rkt/request/trait.FromParam.html
...
:::tip[rkt makes it even _easier_ to serve static files!]

If you need to serve static files from your rkt application, consider using
...
[`FileServer`]: https://docs.rs/rkt/latest/rkt/fs/struct.FileServer.html
...
type mismatch occurs, rkt _forwards_ the request to the next matching route,
...
rank. If one is not specified, rkt chooses a default rank designed to avoid
...
`user_int` routes, rkt would emit an error and abort launch, indicating that
the routes _collide_, or can match against similar incoming requests. The `rank`
parameter resolves this collision.

### Default Ranking

If a rank is not explicitly specified, rkt assigns a default rank to a route.
...
Request guards are one of rkt's most powerful instruments. As the name might
...
handler. rkt only dispatches requests to a handler when all of its guards
pass.
...
The reasoning is as follows:
  ...
  3. Only rkt can provide an active `&Request` to construct via `FromRequest`.
...
[`mtls::Certificate`]: https://docs.rs/rkt/latest/rkt/mtls/struct.Certificate.html
[`mtls::Error`]: https://docs.rs/rkt/latest/rkt/mtls/enum.Error.html
...
other words, the value set is visible to the client. For sensitive data, rkt
...
```toml
## in Cargo.toml
rkt = { version = "1.3.0", features = ["secrets"] }
```
...
[`CookieJar::add()`]: https://docs.rs/rkt/latest/rkt/http/struct.CookieJar.html#method.add
...
is generated automatically. In release mode, rkt requires you to set a secret
...
`PATCH`), the `format` route parameter instructs rkt to check against the
...
`OPTIONS`) the `format` route parameter instructs rkt to check against the
...
rkt would match the `format` against the `Content-Type` header of the
incoming response.

[`ContentType::parse_flexible()`]: https://docs.rs/rkt/latest/rkt/http/struct.ContentType.html#method.parse_flexible

## Body Data

Body data processing, like much of rkt, is type directed. To indicate that a
...
The [`Json<T>`](https://docs.rs/rkt/latest/rkt/serde/json/struct.Json.html) guard deserializes body
...
:::warning[Using rkt's `serde` derive re-exports requires a bit more effort.]
...
rkt intentionally places JSON support, as well support for other data
formats and features, behind feature flags. See [the api
docs](https://docs.rs/rkt/latest/rkt/#features) for a list of available features. The `json`
feature can be enabled in the `Cargo.toml`:

```toml
rkt = { version = "1.3.0", features = ["json"] }
```
:::
...
want to stream the incoming data to some sink. rkt makes this as simple as
possible via the [`Data`](https://docs.rs/rkt/latest/rkt/data/struct.Data.html) type:
...
:::note[rkt requires setting limits when reading incoming data.]

To aid in preventing DoS attacks, rkt requires you to specify, as a
[`ByteUnit`](https://docs.rs/rkt/latest/rkt/data/struct.ByteUnit.html), the amount of data you're
willing to accept from the client when `open`ing a data stream. The
[`ToByteUnit`](https://docs.rs/rkt/latest/rkt/data/trait.ToByteUnit.html) trait makes specifying
...
rkt makes handling them easy. rkt supports both `multipart` and
...
can easily handle the form request in rkt as follows:
...
[`FromFormField`]: https://docs.rs/rkt/latest/rkt/form/trait.FromFormField.html

### Parsing Strategy

rkt's `FromForm` parsing is _lenient_ by default: a `Form<T>` will parse
...
[`Form<Strict<T>>`]: https://docs.rs/rkt/latest/rkt/form/struct.Strict.html
[`Lenient`]: https://docs.rs/rkt/latest/rkt/form/struct.Lenient.html
...
[`Errors<'_>`]: https://docs.rs/rkt/latest/rkt/form/struct.Errors.html
[`form::Result`]: https://docs.rs/rkt/latest/rkt/form/type.Result.html
[`FromForm` derive]: https://docs.rs/rkt/latest/rkt/derive.FromForm.html

### Field Renaming

By default, rkt matches the name of an incoming form field to the name of a
...
expected. You can ask rkt to look for a different form field for a given
...
renamings is allowed. rkt will emit an error at compile-time if field names
...
The expression `range(21..)` is a call to [`form::validate::range`]. rkt
...
[`form::validate`]: https://docs.rs/rkt/latest/rkt/form/validate/index.html
[`form::validate::range`]: https://docs.rs/rkt/latest/rkt/form/validate/fn.range.html
...
[`try_with`]: https://docs.rs/rkt/latest/rkt/form/validate/fn.try_with.html

### Collections

rkt's form support allows your application to express _any_ structure with
_any_ level of nesting and collection, eclipsing the expressivity offered by any
other web framework. To parse into these structures, rkt separates a field's
...
rkt _pushes_ form fields to `FromForm` types as they arrive. The type then
...
declared as `param`. In particular, rkt will expect the query form to contain
...
If any of these occur, rkt returns an error to the client. To generate the
error, rkt invokes the _catcher_ corresponding to the error's status code and
...
Also as with routes, rkt needs to know about a catcher before it is used to
...
rkt provides a built-in default catcher. It produces HTML or JSON, depending
on the value of the `Accept` header. As such, custom catchers only need to be
registered for custom error handling.

The [error handling example](https://github.com/rustfoo/rkt/tree/main/examples/error-handling) illustrates catcher use in
full, while the [`Catcher`] API documentation provides further details.

[`catch`]: https://docs.rs/rkt/latest/rkt/attr.catch.html
[`register()`]: https://docs.rs/rkt/latest/rkt/struct.Rocket.html#method.register
[`mount()`]: https://docs.rs/rkt/latest/rkt/struct.Rocket.html#method.mount
[`catchers!`]: https://docs.rs/rkt/latest/rkt/macro.catchers.html
[`&Request`]: https://docs.rs/rkt/latest/rkt/struct.Request.html
[`Status`]: https://docs.rs/rkt/latest/rkt/http/struct.Status.html
[`Catcher`]: https://docs.rs/rkt/latest/rkt/catcher/struct.Catcher.html
