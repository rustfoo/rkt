...
rkt provides primitives to build web servers and applications with Rust:
rkt provides routing, pre-processing of requests, and post-processing of
responses; the rest is up to you. Your application code instructs rkt on what
to pre-process and post-process and fills the gaps between pre-processing and
post-processing.

## Lifecycle

rkt's main task is to listen for incoming web requests, dispatch the request
...
lifecycle as the following sequence of steps:
  ...
  1. **Routing**
     ...
     rkt parses an incoming HTTP request into native structures that your
     code operates on indirectly. rkt determines which request handler to
     invoke by matching against route attributes declared in your application.

  2. **Validation**

     rkt validates the incoming request against types and guards present in
     the matched route. If validation fails, rkt _forwards_ the request to
  ...
  4. **Response**
     ...
     The returned `Response` is processed. rkt generates the appropriate HTTP
     response and sends it to the client. This completes the lifecycle. rkt
...
components needed for rkt to begin dispatching requests to request handlers.
The sections following describe the request and response phases as well as other
components of rkt.

## Routing

rkt applications are centered around routes and handlers. A _route_ is a
combination of:
...
segments, forms, query strings, request format specifiers, and body data. rkt
...
Throughout this guide and the majority of rkt's documentation, we import
`rkt` explicitly with `#[macro_use]` even though the Rust 2018 edition
makes explicitly importing crates optional. However, explicitly importing with
`#[macro_use]` imports macros globally, allowing you to use rkt's macros
...
Before rkt can dispatch requests to a route, the route needs to be _mounted_:
...
This creates a new `rkt` instance via the `build` function and mounts the
`world` route to the `/hello` base path, making rkt aware of the route.
`GET` requests to `/hello/world` will be directed to the `world` function.

The `mount` method, like all other builder methods on `rkt`, can be chained
...
rkt begins serving requests after being _launched_, which starts a
multi-threaded asynchronous server and dispatches requests to matching routes as
they arrive.

There are two mechanisms by which a `rkt` can be launched. The first and
...
Special to rkt's `#[launch]` attribute, the return type of a function
...
can be found on [GitHub](https://github.com/rustfoo/rkt/tree/main/examples/hello).
You can find other complete examples, spanning all of rkt's features, in the
[GitHub examples directory](https://github.com/rustfoo/rkt/tree/main/examples).
:::
...
[error handling example]: https://github.com/rustfoo/rkt/tree/main/examples/error-handling

## Futures and Async

rkt uses Rust [`Future`]s for concurrency. Asynchronous programming with
...
synchronous equivalents inside rkt applications.

`async` appears in several places in rkt:

* [Routes] and [Error Catchers] can be `async fn`s. Inside an `async fn`, you
  can `.await` `Future`s from rkt or other libraries.
* Several of rkt's traits, such as [`FromData`] and [`FromRequest`], have
...
[`DataStream`]: https://docs.rs/rkt/latest/rkt/data/enum.DataStream.html
[Routes]: ./requests/
...
rkt uses the tokio runtime. The runtime is started for you if you
use `#[launch]` or `#[rkt::main]`, but you can still `launch()` a rkt
instance on a custom-built runtime by not using _either_ attribute.
:::

### Async Routes

rkt makes it easy to use `async/await` in routes.
