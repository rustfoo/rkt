...
[`CliColors`]: https://docs.rs/rkt/latest/rkt/config/enum.CliColors.html
[`TlsConfig`]: https://docs.rs/rkt/latest/rkt/tls/struct.TlsConfig.html
[`ShutdownConfig`]: https://docs.rs/rkt/latest/rkt/shutdown/struct.ShutdownConfig.html
[`ShutdownConfig::default()`]: https://docs.rs/rkt/latest/rkt/shutdown/struct.ShutdownConfig.html#fields
...
[`Config::default()`]: https://docs.rs/rkt/latest/rkt/struct.Config.html#method.default
...
Security). To enable TLS support:
  ...
  1. Enable the `tls` crate feature in `Cargo.toml`:
     ...
     ```toml,ignore
     [dependencies]
     rkt = { version = "1.3.0", features = ["tls"] }
     ```
...
[`CipherSuite`]: https://docs.rs/rkt/latest/rkt/tls/enum.CipherSuite.html
[prefer server cipher suites]: https://docs.rs/rkt/latest/rkt/tls/struct.TlsConfig.html#method.with_preferred_server_cipher_order
...
enabled and support configured via the `tls.mutual` config parameter:
  ...
  1. Enable the `mtls` crate feature in `Cargo.toml`:
   ...
   ```toml,ignore
   [dependencies]
   rkt = { version = "1.3.0", features = ["mtls"] }
   ```
...
[`Request::proxy_proto()`]: https://docs.rs/rkt/latest/rkt/request/struct.Request.html#method.proxy_proto
...
the full header map to be materialized and formatted on _every_ request when
`debug` logging is enabled. To avoid paying that cost unintentionally, header logging is gated
...
Enabling `debug_headers` logs _all_ request and response headers, including
sensitive ones such as `Authorization` and `Cookie`. This can leak credentials
and session tokens into application logs. Avoid enabling it in production, and
treat any logs captured while it is enabled as sensitive.
:::
...
[`Rocket::figment()`]: https://docs.rs/rkt/latest/rkt/struct.Rocket.html#method.figment
...
[`rkt::custom()`]: https://docs.rs/rkt/latest/rkt/fn.custom.html
[`rkt::build()`]: https://docs.rs/rkt/latest/rkt/fn.build.html
