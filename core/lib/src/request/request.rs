...
pub struct Request<'r> {
    ...
    /// covariant (enforced there by `static_assert_covariance!`). Every entry
    /// actually borrows from `header_src` (a `&'r ..`) or from values inserted
    /// through the `&'r mut` view, so the lifetime is narrowed back to `'r` at
    /// every access in `headers`/`headers_mut`.
    ///
    /// SAFETY-CRITICAL: the `'static` here is a lie told only to the variance
    /// checker. It is sound iff every entry is valid for `'r`, which holds
    /// because the only insertion paths are lazy initialization from
    /// `header_src` and `add_header`/`replace_header` (bounded `'h: 'r`), and
    /// because `headers_mut` is private. Changing any of those invalidates the
    /// transmutes in `headers`/`headers_mut`.
...
impl<'r> Request<'r> {
    ...
    ///
    /// SAFETY-CRITICAL: This must remain private. The soundness of the
    /// lifetime-widened `headers` cell depends on every value inserted into
    /// the map being valid for `'r`; the public insertion paths
    /// ([`Request::add_header`], [`Request::replace_header`]) guarantee this
    /// via their `'h: 'r` bounds. Exposing this method (or any other
    /// `&mut HeaderMap` view) publicly would let callers bypass that bound.
    #[inline(always)]
    ...
    pub fn add_header<'h: 'r, H: Into<Header<'h>>>(&mut self, header: H) {
        // SAFETY-CRITICAL: The `'h: 'r` bound is required for the soundness of
        // the lifetime transmutes in `headers`/`headers_mut`; see the comment
        // on the `headers` field. Do not relax it.
