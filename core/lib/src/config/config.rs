...
pub struct Config {
    ...
    /// `ROCKET_DEBUG_HEADERS=true` or `debug_headers = true` in `Rocket.toml`.
    ///
    /// **(default: `false`)**
...
impl Config {
    ...
    /// The stringy parameter name for setting/extracting [`Config::debug_headers`].
    pub const DEBUG_HEADERS: &'static str = "debug_headers";
