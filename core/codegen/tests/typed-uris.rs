...
#[post(
    "/name/<name>?<myfoo>&type=10&<type>&<query..>",
    data = "<user>",
    rank = 2
)]
fn complex<'r>(
...
fn check_route_prefix_suffix() {
    assert_uri_eq! {
        ...
        uri!("/myfoo/") => "/myfoo/",
        uri!("/myfoo///") => "/myfoo/",
        uri!("/myfoo/bar/") => "/myfoo/bar/",
    }

    assert_uri_eq! {
        uri!("http://example.com", index) => "http://example.com",
        uri!("http://example.com/", index) => "http://example.com/",
        uri!("http://example.com///", index) => "http://example.com/",
        uri!("http://example.com/myfoo", index) => "http://example.com/myfoo",
        uri!("http://example.com/myfoo/", index) => "http://example.com/myfoo/",
        uri!("http://", index) => "http://",
        uri!("http:///", index) => "http:///",
        uri!("http:////", index) => "http:///",
    ...
    assert_uri_eq! {
        uri!("http://example.com", index, "?myfoo") => "http://example.com?myfoo",
        uri!("http://example.com", index, "?") => "http://example.com?",
        uri!("http://example.com", index, "#") => "http://example.com#",
        uri!("http://example.com", index, "#bar") => "http://example.com#bar",
        uri!("http://example.com", index, "?bar#baz") => "http://example.com?bar#baz",
        uri!("http://example.com/", index, "?myfoo") => "http://example.com/?myfoo",
        uri!("http://example.com/", index, "?") => "http://example.com/?",
        uri!("http://example.com/", index, "#") => "http://example.com/#",
        uri!("http://example.com/", index, "#bar") => "http://example.com/#bar",
        uri!("http://example.com/", index, "?bar#baz") => "http://example.com/?bar#baz",
        uri!("http://", index, "?myfoo") => "http://?myfoo",
        uri!("http://example.com", simple3(id = 100), "?myfoo") => "http://example.com?id=100",
        uri!("http://example.com", simple3(id = 100), "?myfoo#bar") =>
            "http://example.com?id=100#bar",
        uri!("http://example.com/", simple3(id = 100), "?myfoo") => "http://example.com/?id=100",
        uri!("http://example.com/", simple3(id = 100), "?myfoo#bar") =>
            "http://example.com/?id=100#bar",
        uri!(_, simple3(id = 100), "?myfoo#bar") => "/?id=100#bar",
    }

    let dyn_origin = uri!("/a/b/c");
    let dyn_origin2 = uri!("/a/b/c?myfoo-bar");
    ...
    let dyn_absolute = uri!("http://example.com");
    let dyn_absolute_slash = uri!("http://example.com/");
    assert_uri_eq! {
        uri!(dyn_absolute.clone(), index) => "http://example.com",
        uri!(dyn_absolute.clone(), simple(100)) => "http://example.com/100",
        uri!(dyn_absolute.clone(), simple3(123)) => "http://example.com?id=123",
        uri!(dyn_absolute_slash.clone(), index) => "http://example.com/",
        uri!(dyn_absolute_slash.clone(), simple(100)) => "http://example.com/100",
        uri!(dyn_absolute_slash.clone(), simple3(123)) => "http://example.com/?id=123",
        uri!(uri!("http://example.com/a/b"), index) => "http://example.com/a/b",
        uri!("http://example.com/a/b") => "http://example.com/a/b",
        uri!(uri!("http://example.com/a/b"), index) => "http://example.com/a/b",
        uri!("http://example.com/a/b") => "http://example.com/a/b",
    }

    let dyn_abs = uri!("http://example.com?myfoo");
    assert_uri_eq! {
        uri!(_, index, dyn_abs.clone()) => "/?myfoo",
        uri!("http://example.com", index, dyn_abs.clone()) => "http://example.com?myfoo",
        uri!("http://example.com/", index, dyn_abs.clone()) => "http://example.com/?myfoo",
        uri!("http://", index, dyn_abs.clone()) => "http://?myfoo",
    ...
    let dyn_ref = uri!("?myfoo#bar");
    assert_uri_eq! {
        uri!(_, index, dyn_ref.clone()) => "/?myfoo#bar",
        uri!("http://example.com", index, dyn_ref.clone()) => "http://example.com?myfoo#bar",
        uri!("http://example.com/", index, dyn_ref.clone()) => "http://example.com/?myfoo#bar",
        uri!("http://", index, dyn_ref.clone()) => "http://?myfoo#bar",
        uri!("http:///", index, dyn_ref.clone()) => "http:///?myfoo#bar",
...
fn check_complex() {
    assert_uri_eq! {
        uri!(complex("no idea", 10, "high", ("A B C", "a c"))) =>
            "/name/no%20idea?myfoo=10&type=10&type=high&name=A%20B%20C&nickname=a%20c",
        uri!(complex("Bob", 248, "?", User { name: "Robert", nickname: "Bob".into() })) =>
            "/name/Bob?myfoo=248&type=10&type=%3F&name=Robert&nickname=Bob",
        uri!(complex("Bob", 248, "a a", &User { name: "Robert", nickname: "B".into() })) =>
            "/name/Bob?myfoo=248&type=10&type=a%20a&name=Robert&nickname=B",
        uri!(complex("no idea", 248, "", &User { name: "A B", nickname: "A".into() })) =>
            "/name/no%20idea?myfoo=248&type=10&type=&name=A%20B&nickname=A",
        uri!(complex("hi", 3, "b", &User { name: "A B C", nickname: "a b".into() })) =>
            "/name/hi?myfoo=3&type=10&type=b&name=A%20B%20C&nickname=a%20b",
        uri!(complex(name = "no idea", myfoo = 10, r#type = "high", query = ("A B C", "a c"))) =>
            "/name/no%20idea?myfoo=10&type=10&type=high&name=A%20B%20C&nickname=a%20c",
        uri!(complex(myfoo = 10, name = "no idea", r#type = "high", query = ("A B C", "a c"))) =>
            "/name/no%20idea?myfoo=10&type=10&type=high&name=A%20B%20C&nickname=a%20c",
        uri!(complex(query = ("A B C", "a c"), myfoo = 10, name = "no idea", r#type = "high", )) =>
            "/name/no%20idea?myfoo=10&type=10&type=high&name=A%20B%20C&nickname=a%20c",
        uri!(complex(query = ("A B C", "a c"), myfoo = 10, name = "no idea", r#type = "high")) =>
            "/name/no%20idea?myfoo=10&type=10&type=high&name=A%20B%20C&nickname=a%20c",
        uri!(complex(query = ("A B C", "a c"), myfoo = 10, name = "no idea", r#type = "high")) =>
            "/name/no%20idea?myfoo=10&type=10&type=high&name=A%20B%20C&nickname=a%20c",
        uri!(complex(myfoo = 3, name = "hi", r#type = "b",
                query = &User { name: "A B C", nickname: "a b".into() })) =>
                "/name/hi?myfoo=3&type=10&type=b&name=A%20B%20C&nickname=a%20b",
        uri!(complex(query = &User { name: "A B C", nickname: "a b".into() },
                myfoo = 3, name = "hi", r#type = "b")) =>
                "/name/hi?myfoo=3&type=10&type=b&name=A%20B%20C&nickname=a%20b",
    }
    ...
    assert_uri_eq! {
        uri!(complex("complex", 0, "high", &user)) =>
            "/name/complex?myfoo=0&type=10&type=high&name=Robert&nickname=Bob",
        uri!(complex("complex", 0, "high", &user)) =>
            "/name/complex?myfoo=0&type=10&type=high&name=Robert&nickname=Bob",
        uri!(complex("complex", 0, "high", user)) =>
            "/name/complex?myfoo=0&type=10&type=high&name=Robert&nickname=Bob",
...
fn test_optional_uri_parameters() {
    ...
    assert_uri_eq! {
        uri!(optionals(
            ...
            bar = &"hi there",
            q1 = Some(10),
            rest = Some(Third { one: "hi there".into(), two: "a b" }),
...
fn test_simple_ignored() {
    ...
    #[get("/hey/hi/<_>/myfoo/<_>")]
    fn ignore_inner_two_b() {}

    assert_uri_eq! {
        uri!(ignore_inner_two(100, "boop")) => "/100/myfoo/boop",
        uri!(ignore_inner_one_a("!?")) => "/hi/!%3F/myfoo",
        uri!(ignore_inner_two_b(&mut 5, "boo")) => "/hey/hi/5/myfoo/boo",
    }

    #[get("/<_>/myfoo/<_>?hi")]
    fn ignore_with_q() {}
    #[get("/hi/<_>/myfoo/<_>?hi&<hey>")]
    fn ignore_with_q2(hey: Option<usize>) {}
    #[get("/hi/<_>/myfoo/<_>?<hi>&<hey>")]
    fn ignore_with_q3(hi: &str, hey: &str) {}

    assert_uri_eq! {
        uri!(ignore_with_q(100, "boop")) => "/100/myfoo/boop?hi",
        uri!(ignore_with_q2("!?", "bop", Some(3usize))) => "/hi/!%3F/myfoo/bop?hi&hey=3",
        uri!(ignore_with_q3(&mut 5, "boo", "hi b", "ho")) => "/hi/5/myfoo/boo?hi=hi%20b&hey=ho",
    }
}
...
fn test_maps() {
    ...
    assert_uri_eq! {
        uri!(hmap(map!["myfoo" => 10])) => "/?bar.k:0=myfoo&bar.v:0=10",
        uri!(hmap(map!["myfoo".to_string() => 10])) => "/?bar.k:0=myfoo&bar.v:0=10",
        uri!(hmap(&map!["myfoo".to_string() => 10])) => "/?bar.k:0=myfoo&bar.v:0=10",
        uri!(hmap(&mut map!["myfoo".to_string() => 10])) => "/?bar.k:0=myfoo&bar.v:0=10",
        uri!(hmap(&map!["myfoo" => 10])) => "/?bar.k:0=myfoo&bar.v:0=10",
    }
    ...
    assert_uri_eq! {
        uri!(bmap(map!["myfoo" => 10])) => "/?bar.k:0=myfoo&bar.v:0=10",
        uri!(bmap(map!["myfoo".to_string() => 10])) => "/?bar.k:0=myfoo&bar.v:0=10",
        uri!(bmap(&map!["myfoo".to_string() => 10])) => "/?bar.k:0=myfoo&bar.v:0=10",
        uri!(bmap(&mut map!["myfoo".to_string() => 10])) => "/?bar.k:0=myfoo&bar.v:0=10",
        uri!(bmap(&map!["myfoo" => 10])) => "/?bar.k:0=myfoo&bar.v:0=10",
    }
}
...
fn test_json() {
    ...
    #[get("/?<json>")]
    fn myfoo(json: Json<Inner<usize>>) {}

    let mut inner = Inner { myfoo: Some(10) };
    assert_uri_eq! {
        uri!(myfoo(inner)) => "/?json=%7B%22myfoo%22:10%7D",
        uri!(myfoo(&inner)) => "/?json=%7B%22myfoo%22:10%7D",
        uri!(myfoo(&mut inner)) => "/?json=%7B%22myfoo%22:10%7D",
        uri!(myfoo(Json(inner))) => "/?json=%7B%22myfoo%22:10%7D",
        uri!(myfoo(&Json(inner))) => "/?json=%7B%22myfoo%22:10%7D",
        uri!(myfoo(&mut Json(inner))) => "/?json=%7B%22myfoo%22:10%7D",
    }
    ...
    let mut inner = Inner {
        myfoo: Some(Inner { myfoo: Some("hi") }),
    };
    assert_uri_eq! {
        uri!(bar(inner)) => "/?json=%7B%22myfoo%22:%7B%22myfoo%22:%22hi%22%7D%7D",
        uri!(bar(&inner)) => "/?json=%7B%22myfoo%22:%7B%22myfoo%22:%22hi%22%7D%7D",
        uri!(bar(&mut inner)) => "/?json=%7B%22myfoo%22:%7B%22myfoo%22:%22hi%22%7D%7D",
        uri!(bar(Json(inner))) => "/?json=%7B%22myfoo%22:%7B%22myfoo%22:%22hi%22%7D%7D",
        uri!(bar(&Json(inner))) => "/?json=%7B%22myfoo%22:%7B%22myfoo%22:%22hi%22%7D%7D",
        uri!(bar(&mut Json(inner))) => "/?json=%7B%22myfoo%22:%7B%22myfoo%22:%22hi%22%7D%7D",
    }
}
...
fn test_route_uri_normalization_with_prefix() {
    ...
    assert_uri_eq! {
        ...
        uri!("/myfoo", index()) => "/myfoo",
        uri!("/bar/", index()) => "/bar/",
        uri!("/myfoo/bar", index()) => "/myfoo/bar",
        uri!("/myfoo/bar/", index()) => "/myfoo/bar/",

        uri!("/", world()) => "/world",
        uri!("/myfoo", world()) => "/myfoo/world",
        uri!("/bar/", world()) => "/bar/world",
        uri!("/myfoo/bar", world()) => "/myfoo/bar/world",
        uri!("/myfoo/bar/", world()) => "/myfoo/bar/world",
    }
}
...
fn test_either() {
    ...
    assert_uri_eq! {
        ...
        uri!(f(_myfoo = Left::<usize, &str>(710))) => "/710",

        uri!(f(Right::<usize, &str>("hello world"))) => "/hello%20world",
        uri!(f(_myfoo = Right::<usize, &str>("bye?"))) => "/bye%3F",
    }
}
