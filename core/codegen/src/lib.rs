#![recursion_limit = "128"]
#![doc(html_root_url = "https://docs.rs/rkt_codegen/latest/rkt_codegen")]
...
//! # rkt - Code Generation
//!
//! This crate implements the code generation portions of rkt. This includes
...
//! thoroughly in the [rkt programming guide](https://rkt.rs/guide).
//!
//! # Usage
//!
...
//! example, you might run the following to build a rkt application with
...
macro_rules! route_attribute {
    ($name:ident => $method:expr) => {
        ...
        /// parameter is required to implement one of rkt's guard traits. The
...
/// ```
///
/// The attribute rewrites the function to execute inside of a rkt-compatible
/// async runtime.
#[proc_macro_attribute]
...
/// into a regular `main` function that internally initializes a rkt-specific
...
///     println!("Hello, rkt: {:?}", rocket);
///
///     let rocket = rocket.launch().await?;
///     println!("Welcome back, rkt: {:?}", rocket);
///
///     Ok(())
/// }
/// ```
...
///                 Content-Type, as defined by rkt
/// ```
///
...
/// const ROOT_CONST: Absolute<'static> = uri!("https://example.com");
/// static ROOT_STATIC: Absolute<'static> = uri!("https://example.com?root");
///
/// // Any variant can be parsed, but beware of ambiguities.
/// let asterisk = uri!("*");
/// let origin = uri!("/foo/bar/baz");
/// let authority = uri!("example.com:443");
/// let absolute = uri!("https://example.com:443");
...
/// let bob = uri!("https://example.com", person("Bob", Some(28)));
/// assert_eq!(bob.to_string(), "https://example.com/person/Bob?age=28");
///
/// // with another absolute URI as a prefix
/// let bob = uri!("https://example.com/foo", person("Bob", Some(28)));
/// assert_eq!(bob.to_string(), "https://example.com/foo/person/Bob?age=28");
///
...
/// let bob = uri!("https://example.com/", person("Bob", Some(28)), "#woo");
/// assert_eq!(bob.to_string(), "https://example.com/person/Bob?age=28#woo");
///
...
/// in part `P`. For example, the following implementation, provided by rkt,
...
/// Private rkt internal macro: `internal_guide_tests!`.
#[proc_macro]
#[doc(hidden)]
...
/// Private rkt internal macro: `export!`.
#[proc_macro]
#[doc(hidden)]
...
/// Private rkt attribute: `async_bound(Bounds + On + Returned + Future)`.
#[doc(hidden)]
#[proc_macro_attribute]
